...
class PostController extends Controller
{
    ...
    function store(Request $request){
        ...
        $post->title_slug = Str::slug($request_params["title"]); // Generate slug from title
        $post->body = $request_params["body"];
        $post->posted_by = $request_params["posted_by"];
        $post->image = $file_path;

        $post->save();
        return redirect()->route('post.show', $post->title_slug); // Redirect to the show page using slug
    }
